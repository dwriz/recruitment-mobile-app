...
export default function JobVacancyScreen({ navigation }) {
  ...
  async function fetchData(page) {
    ...
    try {
      const response = await fetch(
        `${process.env.EXPO_PUBLIC_API_URL}/my-job-vacancies?page=${page}&limit=5`,
        {
          method: "GET",
          headers: {
  ...
  function renderCard({ item }) {
    return (
      <TouchableOpacity
        style={styles.card}
        onPress={() => navigation.navigate("JobVacancyDetailScreen", { id: item.id })}
      >
