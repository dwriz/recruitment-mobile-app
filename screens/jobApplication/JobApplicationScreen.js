  ...
  async function fetchData(page) {
    ...
    try {
      const response = await fetch(
        `${process.env.EXPO_PUBLIC_API_URL}/job-applications?page=${page}&limit=5`,
        {
          method: "GET",
          headers: {
      ...
      if (result.statusCode === 200) {
        setData(result.data.job_applications);
    ...
    return (
      <View style={styles.card}>
        ...
        <View style={styles.row}>
          ...
          <Text style={styles.value}>{item.status_hiring_process}</Text>
        </View>
        <View style={styles.row}>
